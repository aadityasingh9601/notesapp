...
1. **State Management**: 
   ...
   - Two handler functions are defined in App: `saveNote` and `deleteNote`

2. **Adding Notes (Upward Flow)**:
   - `NoteForm` maintains local state (`formData`) for form inputs
   - On form submission, `NoteForm` calls the `saveNote` function passed as a prop from App
   - App updates the `notes` array, adding the new note with a generated ID

3. **Displaying Notes (Downward Flow)**:
   - App passes the `notes` array as a prop to `NoteList`
   - `NoteList` maps through the notes and renders a `NoteItem` for each note
   - Each `NoteItem` receives individual note data as props

4. **Deleting Notes (Upward Flow)**:
   - `deleteNote` function is passed from App → NoteList → NoteItem
   - When delete button is clicked in `NoteItem`, it calls `deleteNote` with the note's ID
